...
## v0.1.7 (TBD)
- Modified _display/focus set_ selection behavior in the neural units table. If a modifier key (`Ctrl/Command` or 
`Shift`) is held down when a table row is left-clicked, the corresponding unit is now **removed** from the focus set if 
it was already in that set. Otherwise, as before, it is added to the focus set if there is room (maximum of 3 units can 
